...
public class GoldPassenger extends Passenger {
    ...
    private final Logger LOG = LoggerFactory.getLogger(GoldPassenger.class);

    private double balance;

    public GoldPassenger(int passengerNumber, String name, double balance, ActivityService activityService, DestinationService destinationService, TravelPackageService travelPackageService) {
        super(passengerNumber, name, destinationService, travelPackageService, activityService);
        this.balance = balance;
    }
    /**
     * Allows Gold Passenger to enroll into an Activity at a particular Destination post subscribing to a Travel Package.
     * A Gold Passenger is offered 10% discount on the Activity Cost.
    ...
    public boolean enrollInActivity(String destinationName, String activityName) throws Exception {
        ...
        if (hasSubscribedToPackage && this.destinationService.checkActivityExists(destinationName, activity)) {
            ...
            double currentActivityCost = activity.getActivityCost();
            double costAfterDiscount = currentActivityCost - (currentActivityCost * 0.10);

            if (this.balance - costAfterDiscount > 0 && currentActivityCapacity - 1 > 0) {
                enrolledActivities.add(activity);
                this.balance -= costAfterDiscount;
    ...
    public void printPassengerDetails() {
        ...
        this.enrolledActivities.forEach(activity -> {
            ...
            LOG.info("Price paid for the activity {}", activity.getActivityCost() - (activity.getActivityCost() * 0.10));
        });
    }
